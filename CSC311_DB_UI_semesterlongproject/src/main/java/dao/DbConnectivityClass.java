...
public class DbConnectivityClass {
    final static String DB_NAME="NEWDB";
        MyLogger lg= new MyLogger();
        final static String SQL_SERVER_URL = "jdbc:mysql://csc311sandoval27.mysql.database.azure.com/";//update this server name
        final static String DB_URL = SQL_SERVER_URL + "DBname";//update this database name
        ...
        public boolean connectToDatabase() {
            ...
            try {
                ...
                String sql = "CREATE TABLE IF NOT EXISTS newTable (" + "id INT( 10 ) NOT NULL PRIMARY KEY AUTO_INCREMENT,"
                ...
                statement = conn.createStatement();
                ResultSet resultSet = statement.executeQuery("SELECT COUNT(*) FROM newTable");
        ...
        public void queryUserByLastName(String name) {
            ...
            try {
                Connection conn = DriverManager.getConnection(DB_URL, USERNAME, PASSWORD);
                String sql = "SELECT * FROM newTable WHERE last_name = ?";
                PreparedStatement preparedStatement = conn.prepareStatement(sql);
        ...
        public void insertUser(Person person) {
            ...
            try {
                Connection conn = DriverManager.getConnection(DB_URL, USERNAME, PASSWORD);
                String sql = "INSERT INTO newTable (first_name, last_name, department, major, email, imageURL) VALUES (?, ?, ?, ?, ?, ?)";
                PreparedStatement preparedStatement = conn.prepareStatement(sql);
        ...
        public void editUser(int id, Person p) {
            ...
            try {
                Connection conn = DriverManager.getConnection(DB_URL, USERNAME, PASSWORD);
                String sql = "UPDATE newTable SET first_name=?, last_name=?, department=?, major=?, email=?, imageURL=? WHERE id=?";
                PreparedStatement preparedStatement = conn.prepareStatement(sql);
        ...
        public int retrieveId(Person p) {
            ...
            try {
                Connection conn = DriverManager.getConnection(DB_URL, USERNAME, PASSWORD);
                String sql = "SELECT id FROM newTable WHERE email=?";
                PreparedStatement preparedStatement = conn.prepareStatement(sql);
